...
{
  float r = texture2D(iChannel0, vec2(1.1, 1.3)).r + 0.5;
  float n = max(0., texture2D(iChannel0, vec2(0.15, 0.)).r * 3.5 - 1.);

  n = n * exp(snoise(vec4(p * 2.1, iGlobalTime * 2.3)));
...
void main()
{
  ...
  // camera movement
  vec3 ro, ta;
  doCamera( ro, ta, iGlobalTime, iGlobalTime / 10);
